...
def main(file1,file2):
    ...
    flist=[file1,file2]
    pycode_list = [(f.name, f.read()) for f in flist]
    try:
    ...
    for index, func_ast_diff_list in results:
        ...
        #rstr='ref: {}'.format(pycode_list[0][0])+'\n'+'candidate: {}'.format(pycode_list[index][0])+'\n'
        rstr= '{:.2f} % ({}/{}) of ref code structure is plagiarized by candidate.'.format(
            sum_plagiarism_count / float(sum_total_count) * 100,
            sum_plagiarism_count,
            sum_total_count)
        output_count = 0
